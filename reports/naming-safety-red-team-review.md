...
- `reports/requirement-gap-closure.md:15-27` lists old IDs such as `IBM-APF-001`, `IBM-ZACS-001`, `IBM-SMF-001`, and `IBM-DFSMS-002`.
- `reports/source-card-pin-audit.md:57-65` lists old IBM card IDs for pin review.
- `reports/source-card-audit.md:52-97` lists old `docs/design/source-matrix/cards/IBM-*.yml` paths.
- `reports/source-card-targeted-review.md:29-53` uses old IBM IDs in review tables.

`scripts/check-extref-namespace.py:64-83` does not scan `reports/` at all, even though its allowlist at `scripts/check-extref-namespace.py:20-31` only names `reports/naming-*` as a safe old-ID context.
