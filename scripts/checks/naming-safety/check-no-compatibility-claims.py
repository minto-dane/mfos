...
EXTERNAL_RE = re.compile(r"\b(IBM|z/OS|RACF|JES2?|DFSMS|SMF|MVS|IBM Z|z/Architecture|APF|PR/SM|LPAR|SDSF|ISPF|TSO/E|TSO|zOSMF|z/OSMF|Hyper[- ]?V|KVM|TDX|SEV(?:[- ]ES|[- ]SNP)?|SGX|CET|CFI|Kani|Verus|CodeQL|NIST)\b", re.IGNORECASE)
RISK_RE = re.compile(
    r"\b("
    ...
    r"|equivalent\s+to"
    r"|replacement\s+for"
    r"|(?:certified|approved|endorsed|sponsored)\s+by\s+(?:IBM|Microsoft|Linux|KVM|Hyper[- ]?V|Intel|AMD|TDX|SEV(?:[- ]ES|[- ]SNP)?|SGX|CET|CFI|Kani|Verus|CodeQL|NIST)"
    r"|(?:IBM|Microsoft|Linux|KVM|Hyper[- ]?V|Intel|AMD|TDX|SEV(?:[- ]ES|[- ]SNP)?|SGX|CET|CFI|Kani|Verus|CodeQL|NIST)\s+(?:certified|approved|endorsed|sponsored)"
    r")\b",
    re.IGNORECASE,
)
