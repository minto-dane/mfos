...
def main() -> int:
    ...
    generated_names = {Path(path).name for path in generated_entries}
    ...
    for path in sorted(REPORT_CURRENT.rglob("*") if REPORT_CURRENT.exists() else []):
        ...
        if path.suffix == ".md" and path.name in generated_names:
            findings.append(Finding("ERROR", path, "generated Markdown report artifact must not be under reports/current"))
