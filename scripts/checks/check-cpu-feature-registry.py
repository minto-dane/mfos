...
SGX_FORBIDDEN = {"X64_FEATURE_INTEL_TDX", "X64_FEATURE_AMD_SEV_SNP"}
...
def main() -> int:
    ...
    for feature_id, entry in features.items():
        ...
        elif detection.get("design_status") == "implemented":
            findings.append(Finding("ERROR", FEATURE_REGISTRY, f"{feature_id}: design registry must not claim implemented feature detection"))
    ...
    for profile_id, entry in profiles.items():
        ...
        if profile_id == "MFOS-X64-V4-PERFORMANCE":
            if required & V4_UNRELATED:
                findings.append(Finding("ERROR", PROFILE_REGISTRY, "v4 profile must not require TDX/SEV/SGX/CET/CFI"))
        ...
        if profile_id == "MFOS-X64-TEE-INTEL-SGX":
            ...
            if not SGX_FORBIDDEN.issubset(forbidden):
                findings.append(Finding("ERROR", PROFILE_REGISTRY, "SGX profile must forbid TDX and SEV-SNP implication"))
