...
def main() -> int:
    ...
    if set(_strings(v4, "required_features") + _strings(v4, "optional_features")) & v4_unrelated:
    ...
    if set(_strings(sgx, "required_features") + _strings(sgx, "optional_features")) & sgx_cvm_features:
        findings.append(Finding("ERROR", PROFILES, "SGX profile must not imply TDX or SEV-family CVM features"))
    if not sgx_cvm_features.issubset(set(_strings(sgx, "forbidden_features"))):
        findings.append(Finding("ERROR", PROFILES, "SGX profile must explicitly forbid TDX and SEV-family CVM implication"))
